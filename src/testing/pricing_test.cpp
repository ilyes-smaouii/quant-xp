...
int main(int argc, char *argv[]) {
  // Parameters - pt. 1
  my_time_t time_step;
  constexpr std::size_t max_steps_per_asset{};
  // Parameters - pt. 2
  std::size_t inputs_sample_size{}, simulations_per_input{};
  ...
  for (std::size_t i{0}; i < inputs_sample_size; i++) {
    ...
    std::size_t steps_per_asset{curr_input._maturity.ticks()};
    ...
    for (std::size_t j{0}; j < simulations_per_input; j++) {
      auto timestamps_and_prices = simulateAssetPrices(
          curr_input._curr_price, curr_input._risk_free_interest_rate,
      ...
      auto diff = prices[steps_per_asset] - curr_input._strike_price;
      if (diff > 0) {
        total_pnl += diff;
      }
    }
